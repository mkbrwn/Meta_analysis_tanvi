...
# Addition at line 194 (to load readxl package, and then open file)
# Apply in part 4 and 5 (changing reference terms and file names, etc)

# =============================================================================
# Age Data Parsing Script — Version 7
# =============================================================================
#
# CONTENTS
# --------
# Header: description and change log .......................... 1-111
# Libraries .................................................... 172-174
#
# HELPERS
#   normalise_text() ........................................... 179-188
#     Normalise unicode characters to ASCII ................... 181-187
#   extract_category() ......................................... 192-206
#     Match age category patterns from text ................... 194-205
#     FIX (#15): comparison operator without "y" suffix added . 203
#   is_category_only() ......................................... 211-223
#     Check whether cell contains category label only ......... 213-222
#   is_value_semicolon() ....................................... 228-244
#     Early exit: no semicolon ................................ 229
#     Early exit: Q1;Q3 pattern ............................... 230
#     Split and assess segments ............................... 231-243
#       is_stat_fragment check ................................ 235
#       non_stat_words / has_group_name check ................. 237-241
#     Return: all segments value-like? ........................ 243
#   strip_cell_header() ........................................ 249-294
#     Early exit: no separator ................................ 251-254
#     Split on semicolons ..................................... 255-258
#     Detect standard header .................................. 260-262
#     Detect embedded-groups header ........................... 263-271
#       FIX (#10,#20): (?:>=|<=|[>=<]) handles normalised ">=" 265
#     Build and return header/remainder/avg+disp type ......... 272-292
#   extract_bare_avg() ......................................... 300-310
#     Strip (n=X) notation .................................... 302
#     Allow full "years" suffix (#50,56) ...................... 304
#
# PART 1: parse_age_block() .................................. 315-799
#   Output list initialisation ................................ 322-348
#   Early exit: empty/NA text ................................ 350-357
#   Normalise text and extract category ....................... 359-360
#   Flags (approx, IQR+pm) ................................... 362-364
#   Early exit: category-only cell ........................... 366-372
#
#   AVERAGE EXTRACTION ........................................ 374-555
#     Secondary median removal (#104) ......................... 376-384
#     Mean extraction ......................................... 386-450
#       Special case: "mean +/- SD X ± Y" (#2,8,18) .......... 391-398
#       Pattern A: keyword + optional words + number .......... 400-411
#         FIX (#6): "years?" added to optional words .......... 402
#       Pattern B: "Mean 75" / "Mean 49.5y" .................. 413-418
#       Pattern C: "63 (mean)" ............................... 420-425
#       Pattern D: "Mean (SD) X" ............................. 427-436
#       Pattern E: fallback bare number after keyword ......... 438-449
#         FIX (#108): changed ^\\s* to ^[^\\d]* to skip labels  444
#     Median extraction ....................................... 452-500
#       Pattern 1: UNIFIED — (n=X) stripped, [^0-9]*? skip .... 457-469
#         FIX (#14,#19,#56): non-greedy skip all non-digit ..... 463-468
#       Pattern 2: number BEFORE keyword (#99) ................ 471-479
#       Pattern 3: "(Median ± SD)" label pattern (#1) ......... 481-492
#         FIX (#1): extract first number in cell .............. 483-491
#       Pattern 4: approximate "Median ~65" ................... 494-498
#     Secondary median fallback .............................. 501
#     Average not specified .................................. 503-515
#     Assign average flag columns ............................ 517-527
#     Inheritance: avg type from overall/cell ................. 529-555
#       FIX (#90,108): value re-assigned after clearing ........ 543,554
#
#   BUILD post_avg_text ....................................... 558-566
#
#   DISPERSION EXTRACTION ..................................... 569-798
#     Keyword detection flags ................................ 571-574
#     SD extraction ........................................... 576-632
#       Special case: "mean +/- SD X ± Y" (#2,8,18) .......... 579-586
#       "(SD)" label in post_avg_text (#79,84,93) ............. 588-605
#       "SD X" unbracketed ................................... 607-611
#       "± X" / "+/- X" ..................................... 613-618
#       Standalone "+" as SD indicator (#82) .................. 620-624
#       FIX (#1): "(Median ± SD)" SD extraction ............... 626-632
#     IQR extraction .......................................... 634-676
#       Two-number IQR pattern ............................... 639-646
#       Bracketed pair when keyword elsewhere ................. 647-654
#       Single IQR value ..................................... 655-665
#       Inherited IQR: bracketed single value (#75) .......... 666-675
#     Range extraction ........................................ 677-699
#       "ranged/range from X to Y" pattern ................... 682-685
#       "(range) avg (lo-hi)" pattern ........................ 686-696
#       FIX (#86,89): post_avg_text bracket fallback ......... 697-699
#     Unspecified dispersion ................................. 701-725
#       Bare hyphenated pair ................................. 705-709
#       Single bracketed number -> unspec_disp_value ......... 712-717
#       Bracketed pair (lo, hi) .............................. 718-725
#     Assign dispersion flag columns .......................... 727-795
#       SD ................................................... 729-731
#       IQR (+ capture alongside range/SD) ................... 733-740
#       FIX (#86,89): range assignment isTRUE bug fixed ....... 742-745
#       Unspecified .......................................... 747-753
#       dispersion_not_reported .............................. 755
#       Inherit dispersion type .............................. 757-762
#       FIX (#72,75,90,108): move unspec to IQR if inherited .. 764-778
#     Flag: UQR < LQR ......................................... 790
#     Flag: unparsed numbers ................................. 792-797
#   Return output list ........................................ 799
#
#   Utility functions (%||%, coalesce) ........................ 801-808
#
# PART 2: split_groups() ..................................... 811-1033
#   Empty result list ......................................... 816-826
#   Early exit: empty/NA text ................................ 828
#   Normalise and pre-clean text ............................. 830-836
#   Strip and save cell header ............................... 838-841
#   Extract shared category .................................. 843
#   Extract cell-level avg/disp type for inheritance .......... 845-859
#   Detect "overall" keyword segment .......................... 861-872
#   Group splitting patterns ................................. 874-935
#     Pattern F: short label groups "bQ...bM" ................ 876-887
#       FIX (#90): moved before Pattern B to prevent false split
#     Pattern B: (n=X) boundaries ........................... 890-897
#     Pattern C: semicolons .................................. 899-905
#     Pattern D: "vs" ........................................ 907-914
#     Pattern E: sentence split on ". word" .................. 917-935
#       FIX (#82): changed [A-Z] to \\w for lowercase splits .. 920
#       Merge pronoun-led fragments .......................... 926-934
#   Fallback: treat whole cell as overall ..................... 937-940
#   Extract group name (inner function) ....................... 942-1011
#     Strip (n=X) notation ................................... 945
#     FIX (#10,#20): strip category+format prefix from start .. 947-953
#     Check for "(Name)" at end of segment ................... 955-960
#     FIX (#72): check for plain words at end of segment ..... 962-972
#     Strip leading stat keywords ............................ 974-977
#     Bracketed name at start ............................... 979-983
#     Bracketed name mid-segment ............................ 985-991
#     Text before first digit or colon ....................... 993-1009
#   Assign group names and text .............................. 1013-1017
#   Return result list ........................................ 1019-1032
#
# PART 3: parse_age_cell() ................................... 1036-1080
#   Split cell into group blocks ............................. 1039
#   Parse overall block ....................................... 1041
#   Determine inheritance values ............................. 1041-1050
#   Parse group_1 and group_2 blocks ......................... 1052-1059
#   Propagate shared category ................................ 1061-1065
#   Combine and return all results ........................... 1067-1079
#
# PART 4: Apply to tibble .................................... 1083-1092
#   Apply parse_age_cell() to each row ....................... 1083
#   Convert results to tibble ................................ 1085-1087
#   Bind to original tibble .................................. 1089-1090
#
# PART 5: Save to Excel ...................................... 1093-1106
#   Sheet 1: blank NAs ....................................... 1095-1098
#   Sheet 2: "not reported" NAs ............................. 1100-1104
#   Save workbook ............................................ 1106
#
# =============================================================================
# CHANGES VS V6:
#
# EXTRACTION FIXES:
#   #14,#19,#56: Median Pattern 1 completely rewritten as a unified non-greedy
#                skip pattern (\b(?:median|medican)\b[^0-9]*?(\b\d{2,3}...))
#                that handles ALL label formats between keyword and value,
#                including "(Q1 ; Q3)", "(IQR 77...)", "age: GroupName", "[IQR]"
#   #108: Mean Pattern E fallback changed from ^\\s* to ^[^\\d]* so label
#         words like "IQR: empiricial antiviral" are skipped before the number
#
# GROUPING FIXES:
#   #82:  Pattern E sentence split changed from (?=[A-Z]) to (?=\\w) so
#         lowercase group names like "survivors" also become split points
#   #90:  Pattern F (bQ/bM) moved before Pattern B to prevent range bounds
#         like "(44-67)" from triggering a false group split
#   #10,#20: Category+format prefix now stripped from segment start inside
#            extract_group_name, allowing "KP-CAP" and "positive BC" to be
#            correctly extracted from segments that start with ">=18 years: ..."
#   #10,#20: strip_cell_header format-stripping regex changed from [>=<]?
#            (one char) to (?:>=|<=|[>=<]) to correctly consume normalised ">="
#            so embedded-groups detection fires and the header is properly split
#
# CATEGORY FIX:
#   #15:  extract_category now also matches comparison operators without a
#         trailing "y" suffix (e.g. ">18" not just ">18y" or ">18 years")
#
...
strip_cell_header <- function(text) {
  ...
  if (has_category && has_format_word && has_age_number) {
    stripped_trial <- str_remove(first, regex(
      ...
      ignore_case = TRUE))
    if (str_detect(str_trim(stripped_trial), regex("^[A-Za-z].*\\d", ignore_case = FALSE))) {
...
parse_age_block <- function(text,
  ...
  if (has_median_kw) {
    # Pattern 1: UNIFIED — strip (n=X) then skip any non-digit content to find
    # first 2-3 digit plausible age. Handles "(Q1 ; Q3)", "(IQR 77...)", "[IQR]",
    # "age: GroupName 48", and all other label formats between keyword and value.
    # FIX (#14,#19,#56): replaces the fragile optional-bracket approach with a
    # non-greedy skip of ALL non-digit content.
    text_no_n <- str_remove_all(text, regex("\\(\\s*n\\s*=\\s*\\d+[^)]*\\)", ignore_case = TRUE))
    m1 <- str_extract(text_no_n, regex(
      "\\b(?:median|medican)\\b[^0-9]*?(\\b\\d{2,3}(?:\\.\\d+)?\\b)",
      ignore_case = TRUE))
    if (!is.na(m1)) {
      val <- as.numeric(str_extract(m1, "[\\d.]+$"))
...
split_groups <- function(text) {
  ...
  # NOTE: Pattern F (bQ/bM) runs before Pattern B to prevent range bounds like
  # "(44-67)" triggering a false split via the digit+")"+space+letter rule.
  ...
  # Pattern B (n=X) boundaries — only runs when no bQ/bM pattern found
  if (length(group_segs) == 0 &&
  ...
  # Pattern E: sentence split on ". word" (any word character, not just uppercase)
  # FIX (#82): changed [A-Z] to \\w so lowercase group names like "survivors" also
  # trigger a split. The pronoun-merge step then handles "Their" fragments correctly.
  ...
  # (Pattern F moved above Pattern B — see comment there)
  ...
  extract_group_name <- function(seg) {
    ...
    # FIX (#10,#20): strip any leading category+format prefix before extracting name
    # e.g. ">=18 years: years median [IQR] KP-CAP 68" -> "KP-CAP 68"
    # e.g. ">=18 years: positive BC median (range) 62" -> "positive BC median (range) 62"
...
addWorksheet(wb, "age_clean")
writeData(wb, "age_clean", my_tibble_clean)

my_tibble_nr <- my_tibble_clean %>%
  mutate(across(everything(), ~ ifelse(is.na(.), "NR", as.character(.))))

addWorksheet(wb, "age_clean (not reported)")
writeData(wb, "age_clean (not reported)", my_tibble_nr)

saveWorkbook(wb, "age_data_cleaned_v7.xlsx", overwrite = TRUE)