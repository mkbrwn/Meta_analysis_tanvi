...
# Addition at line 48 (to load readxl package, and then open file)
...
parse_age_block <- function(text,
  ...
  # isTRUE() used throughout to safely handle NA values — returns FALSE for NA
  # rather than crashing the if statement with "missing value where TRUE/FALSE needed"
  if (isTRUE(out$avg_not_reported == "Y") && !is.null(inherit_mean) && isTRUE(inherit_mean == "Y")) {
    out$mean_reported    <- "Y"
    out$avg_not_reported <- "N"
  }
  if (isTRUE(out$avg_not_reported == "Y") && !is.null(inherit_median) && isTRUE(inherit_median == "Y")) {
    out$median_reported  <- "Y"
    out$avg_not_reported <- "N"
  }
  ...
  if (!is.null(inherit_disp) && isTRUE(out$dispersion_not_reported == "Y")) {
    if (isTRUE(inherit_disp == "IQR")) out$IQR_reported <- "Y"
    if (isTRUE(inherit_disp == "SD"))  out$SD_reported  <- "Y"
...
parse_age_cell <- function(text) {
  ...
  group_1_data  <- parse_age_block(
    ...
    inherit_disp   = if (isTRUE(overall_data$IQR_reported == "Y")) "IQR" else
      if (isTRUE(overall_data$SD_reported  == "Y")) "SD"  else NULL
  )
