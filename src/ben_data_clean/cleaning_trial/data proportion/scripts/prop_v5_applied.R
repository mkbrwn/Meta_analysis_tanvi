...
output_path     <- "C:/Users/goddab/OneDrive - University Hospital Southampton NHS Foundation Trust/Desktop/RESEARCH_tanvi_cleaning_trial/proportion data/cleaned data/proportion_cleaned_v5.xlsx"
output_path_nr  <- "C:/Users/goddab/OneDrive - University Hospital Southampton NHS Foundation Trust/Desktop/RESEARCH_tanvi_cleaning_trial/proportion data/cleaned data/proportion_cleaned_v5_NR.xlsx"
...
#
# FIX (study 101, 111): "number of participants" had the AGE value
# accidentally copy-pasted into it (e.g. "60 (49-75)" -- identical to the
# age column's own entry for that row), but this was never caught because
# the age column (R) was not part of all_prop_col_names at all -- it's a
# demographic column, not a proportion column, so it had never been
# included in the cross-column comparison. age_col_name is now always
# checked in addition to the other proportion columns, regardless of which
# sheet is currently being processed.
# ---------------------------------------------------------------------------
flag_duplication <- function(cell_nums, row_idx, current_col_names, df,
                             all_prop_col_names, age_col_name = NULL) {
  ...
  if (!is.null(age_col_name) && age_col_name %in% colnames(df) &&
      !(age_col_name %in% current_col_names)) {
    other_cols <- union(other_cols, age_col_name)
  }
...
#
# FIX (big-picture issue): previously there was only ONE set of manual
# checks, computed from a single "primary" value (overall, falling back to
# subgroup1 only when overall was missing) and always checked against the
# OVERALL study total (column S). This was structurally wrong for subgroups:
# e.g. study 4's development-cohort subgroup has pop=455, but column S for
# the whole study is 575 -- comparing 455 to 575 will always flag "N" even
# though the subgroup's own numbers are entirely self-consistent.
#
# The fix: each block (overall, subgroup1, subgroup2) now gets its OWN full
# set of checks. The denominator used is the block's own reported population
# (pop) when available (this is what a subgroup's fraction was actually
# computed from in the source), falling back to the overall study total
# (n_total, column S) only when the block has no pop of its own -- this
# applies naturally to "overall" blocks, which generally SHOULD be checked
# against the whole-study N.
# ---------------------------------------------------------------------------
...
for (col_def in PROPORTION_COLS) {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    checks_sub1    <- compute_checks(parsed$sub1_num, parsed$sub1_prop, parsed$sub1_pop, n_total)
    checks_sub2    <- compute_checks(parsed$sub2_num, parsed$sub2_prop, parsed$sub2_pop, n_total)
    ...
    flag_dup <- flag_duplication(
      raw_nums, i, existing_cols, df, all_prop_col_names, COL_AGE)
    ...
    if (cell_has_subgroups) {
      ...
      has_overall_block <- length(parsed$parsed_blocks) > 0 &&
        parsed$parsed_blocks[[1]]$label == "overall"
      final_has_sub <- TRUE
    ...
    individual_flags <- c(
      parsed$flag_not, parsed$flag_question, parsed$flag_avg_keyword,
      flag_unused, parsed$flag_zero_one, flag_dup, flag_additional_text
    )
    equal_checks <- c(
      checks_overall$prop_equal, checks_overall$num_equal, checks_overall$pop_equal,
      checks_sub1$prop_equal,    checks_sub1$num_equal,    checks_sub1$pop_equal,
      checks_sub2$prop_equal,    checks_sub2$num_equal,    checks_sub2$pop_equal
    )
...
FLAG_COLS  <- c("flag_any","flag_not","flag_question","flag_avg_keyword","flag_unused_numbers",
