...
detect_subgroups <- function(groups_val, g1_val, g2_val) {
  g  <- if (!is.na(groups_val)) str_trim(as.character(groups_val)) else ""
  g1 <- if (!is.na(g1_val))    str_trim(as.character(g1_val))     else ""
  g2 <- if (!is.na(g2_val))    str_trim(as.character(g2_val))     else ""
...
build_study_reference <- function(author_raw, year_raw) {
  author <- if (!is.na(author_raw)) str_trim(as.character(author_raw)) else ""
  year   <- if (!is.na(year_raw))   str_trim(as.character(year_raw))   else ""
