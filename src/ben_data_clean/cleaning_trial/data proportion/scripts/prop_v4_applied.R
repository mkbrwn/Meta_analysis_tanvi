...
output_path     <- "C:/Users/goddab/OneDrive - University Hospital Southampton NHS Foundation Trust/Desktop/RESEARCH_tanvi_cleaning_trial/proportion data/cleaned data/proportion_cleaned_v4.xlsx"
output_path_nr  <- "C:/Users/goddab/OneDrive - University Hospital Southampton NHS Foundation Trust/Desktop/RESEARCH_tanvi_cleaning_trial/proportion data/cleaned data/proportion_cleaned_v4_NR.xlsx"
...
split_into_blocks <- function(text) {
  ...
  # FIX (study 4 type cells): "total n=125/575 (development cohort: 97/455
  # (21.3%); external validation cohort: 28/120 (23.3%))" -- the OVERALL
  # fraction (125/575) is immediately followed by a bracket that belongs to
  # a nested subgroup breakdown, not its own percentage. Without this check,
  # the general N/M-then-% pattern used later skips past 125/575 entirely
  # and matches the first subgroup fraction (97/455) instead, so the overall
  # value was being lost and a subgroup value wrongly treated as overall.
  #
  # This pattern is deliberately narrow (requires "total"/"overall" keyword,
  # then N/M, then an opening bracket containing labelled sub-segments
  # separated by ";") so it only activates for this specific recognised
  # shape and does not change behaviour for any other cell (e.g. study 7's
  # "n=101/257 (39.3%) (Men: ...; Women: ...)" still falls through to the
  # normal logic below, since here the overall percentage is NOT nested --
  # it has its own bracket immediately after the fraction).
  overall_lead <- str_match(s, regex(
    "^(?:total|overall)\\s*n?\\s*[=:]?\\s*(\\d+)\\s*/\\s*(\\d+)\\s*\\((.*;.*)\\)\\s*$",
    ignore_case = TRUE))
